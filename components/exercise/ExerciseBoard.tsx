...
export default function ExerciseBoard() {
  ...
  function resetBoard() {
    setIsPlaced(
      Object.fromEntries(
        CA_COMPONENTS.map((component, _) => [component.id, ""]),
      ),
    );
    setIsFilled(
      Object.fromEntries(
  ...
  function allComponentsPlaced(): boolean {
    return CA_COMPONENTS.every((component) => isPlaced[component.id] !== "");
  }

  function checkWork() {
    if (allComponentsPlaced()) {
      setScore(
        Object.entries(isFilled).filter(
          (componentDroppable) =>
            componentDroppable[0] == componentDroppable[1],
        ).length,
      );
      setIsVerified(true);
    } else {
        ...
        onDragEnd={(event) => {
          ...
          if (event.operation.target?.id == "sidebar-droppable") {
            ...
            if (isPlaced[event.operation.source?.id as string] == "") {
              ...
              setIsFilled({
                ...isFilled,
                [event.operation.target?.id as string]: event.operation.source
                  ?.id as string,
              });
            } else {
              const oldDroppable =
              ...
              if (isFilled[event.operation.target?.id as string] == "") {
                ...
                setIsFilled({
                  ...isFilled,
                  [oldDroppable]: oldDraggable,
                  [event.operation.target?.id as string]: event.operation.source
                    ?.id as string,
                });
                setIsPlaced({
                  ...isPlaced,
                  [oldDraggable]: oldDroppable,
                  [event.operation.source?.id as string]: event.operation.target
                    ?.id as string,
                });
              }
      ...
      >
        ...
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
          <ExerciseBoardSidebar
            ...
            handleRetry={retryBoard}
            handleCheckWork={checkWork}
          />
        </Box>
      </DragDropProvider>
    </>
